 ...
 */
public class BorderEmbossFilter extends WholeImageFilter {
    ...
    protected int[] filterPixels( int width, int height, int[] inPixels, Rectangle transformedSpace ) {
        ...
        for (int i = 0; i < inPixels.length; i++)
            bumpPixels[i] = (inPixels[i] >> 24) & 0xff; // PixelUtils.brightness(inPixels[i]);
        ...
        for (int y = 0; y < height; y++, bumpIndex += bumpMapWidth) {
            ...
            for (int x = 0; x < width; x++, s1++, s2++, s3++) {
                ...
                // don't compute the embossing for fully transparent pixels
                final int a = rgb & 0xff000000;
                if (a == 0) {
                    shade = background;
                } else if (y != 0 && y < height-2 && x != 0 && x < width-2) {
    ...
    public static void main(String [] args) {
        ...
        try {
            ...
            java.io.File file = new java.io.File("/Volumes/FireWireHD/forestmask_gray.png");
            BufferedImage image = javax.imageio.ImageIO.read(file);
            image = com.jhlabs.image.ImageUtils.convertImageToARGB(image);
            
            GaussianFilter gaussianFilter = new GaussianFilter();
            gaussianFilter.setRadius(2);
            image = gaussianFilter.filter(image, null);
            
            BorderEmbossFilter filter = new BorderEmbossFilter();
            filter.setBumpHeight(10);
