...
import edu.oregonstate.carto.mapcomposer.map.Map;
import edu.oregonstate.carto.mapcomposer.map.MapLayer;
import edu.oregonstate.carto.mapcomposer.map.MapTileGenerator;
...
public class MapComposerPanel extends javax.swing.JPanel {
    ...
    /**
     * This map object is the model of this window.
     */
    ...
    /**
     * The tile sets or raster images available in the user interface to create
     * map layers.
     */
    ...
    private MapLayer getSelectedMapLayer() {
        return map.getLayer(layerList.getSelectedIndex());
    }
    ...
    private void initComponents() {
        ...
        tintColorButton = new ika.gui.ColorButton();
        ...
        shadowColorButton = new ika.gui.ColorButton();
        ...
        gridBagConstraints.gridy = 2;
        gridBagConstraints.anchor = java.awt.GridBagConstraints.LINE_END;
        settingsPanel.add(imageLabel, gridBagConstraints);

        imageComboBox.setEditable(true);
        imageComboBox.setMaximumRowCount(25);
        imageComboBox.setModel(new javax.swing.DefaultComboBoxModel(new String[] { "Item 1", "Item 2", "Item 3", "Item 4" }));
        ...
        gridBagConstraints.gridwidth = 3;
        gridBagConstraints.fill = java.awt.GridBagConstraints.HORIZONTAL;
        settingsPanel.add(imageComboBox, gridBagConstraints);
    ...
    private void moveDownLayerButtonActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_moveDownLayerButtonActionPerformed
        ...
        if (selectedLayerID < 0 || selectedLayerID >= this.map.getLayerCount() - 1) {
            return;
        }
        MapLayer layer = this.map.removeLayer(selectedLayerID);
        this.map.addLayer(layer, ++selectedLayerID);
    ...
    private void addLayerButtonActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_addLayerButtonActionPerformed
        ...
        int selectedLayerID = this.layerList.getSelectedIndex();
        this.map.addLayer(new MapLayer(name), ++selectedLayerID);
        this.writeGUI();
        this.layerList.setSelectedIndex(selectedLayerID);
    ...
    final void writeGUI() {
        ...
        try {
            ...
            int selectedID = this.layerList.getSelectedIndex();
            this.layerList.setListData(this.map.getLayers().toArray());
            this.layerList.setSelectedIndex(selectedID);

            MapLayer selectedLayer = getSelectedMapLayer();
            ...
            this.imageComboBox.setEnabled(on);
            ...
            if (selectedLayer == null) {
                ...
                this.imageComboBox.setSelectedItem(null);
            ...
            this.nameTextField.setText(selectedLayer.getNameLayer());
            this.imageComboBox.setSelectedItem(selectedLayer.getImageName());

            // blending
            this.normalBlendingRadioButton.setSelected(selectedLayer.getBlending() == MapLayer.BLENDINGNORMAL);
            this.multiplyBlendingRadioButton.setSelected(selectedLayer.getBlending() == MapLayer.BLENDINGMULTIPLY);
            ...
            // mask
            this.maskComboBox.setSelectedItem(selectedLayer.getMaskName());
    ...
    public void readGUI() {
        ...
        layer.setNameLayer(this.nameTextField.getText());
        layer.setBlending(this.normalBlendingRadioButton.isSelected()
                ? MapLayer.BLENDINGNORMAL : MapLayer.BLENDINGMULTIPLY);
        layer.setOpacity(this.opacitySlider.getValue() / 100.f);
        layer.setCurveURL(this.curveFilePathTextField.getText());

        layer.setImageName((String) this.imageComboBox.getSelectedItem());

        // mask
        layer.setMaskName((String) this.maskComboBox.getSelectedItem());
    ...
    protected void setImageCollection(ImageCollection imageCollection) {
        this.imageCollection = imageCollection;
        
        String[] images = imageCollection.getImageNames();
        imageComboBox.removeAllItems();
        for (String url : images) {
            imageComboBox.addItem(url);
        }
        maskComboBox.removeAllItems();
        for (String url : images) {
            maskComboBox.addItem(url);
        }
    }
     ...
     */
    protected ImageCollection getImageCollection() {
    ...
    private ika.gui.ColorButton shadowColorButton;
    ...
    private ika.gui.ColorButton tintColorButton;
